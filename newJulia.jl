...
const h_cache = Threads.Atomic{Dict{Tuple{String, Int, Int}, Float64}}(Dict())

function h_00(K, i)
    key = ("h_00", K, i)
    h_cache_local = h_cache[]  # Access the atomic dictionary safely
    if haskey(h_cache_local, key)
        return h_cache_local[key]
    end
    f = (x, v) -> f_opt(x) * (K - i) * F_opt(x)^(K - i - 1) * g_sub(v) * i * G_sub(v)^(i - 1)
    result, _ = scipy_integrate.dblquad(f, -Inf, Inf, (v) -> -Inf, (v) -> v)
    Threads.atomic_set!(h_cache, merge(h_cache_local, Dict(key => 1 - result)))  # Update the cache safely
    return h_cache[key]
end

function h_01(K, i)
    key = ("h_01", K, i)
    h_cache_local = h_cache[]
    if haskey(h_cache_local, key)
        return h_cache_local[key]
    end
    f = (x, v) -> f_sub(x) * (K - i) * F_sub(x)^(K - i - 1) * g_opt(v) * i * G_opt(v)^(i - 1)
    result, _ = scipy_integrate.dblquad(f, -Inf, Inf, (v) -> -Inf, (v) -> v)
    Threads.atomic_set!(h_cache, merge(h_cache_local, Dict(key => 1 - result)))
    return h_cache[key]
end
